...
### Model Switching Skills

This installation includes two convenience skills for quickly switching between popular OpenRouter models:

- `/alpha` - Switch to OpenRouter's Owl Alpha model (optimized for agentic workloads)
- `/nemo` - Switch to NVIDIA's Nemotron 3 Super 120B free model via OpenRouter

These skills automatically configure the model and provider, then reload the session to apply changes. Ensure your OpenRouter API key is configured in `~/.hermes/.env` before using them.
...
1. Verify Hermes Agent is installed: `curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash`
2. Check your shell's PATH environment variable to ensure it includes the installation directory (e.g., `~/.local/bin` or `/usr/local/bin`). You can inspect PATH with `echo $PATH`.
3. If PATH is incorrect, add the Hermes installation directory to it (e.g., `export PATH="$HOME/.local/bin:$PATH"` in your shell's rc file like `.bashrc` or `.zshrc`).
