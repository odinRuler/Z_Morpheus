...
description: "Baoyu article illustration umbrella: article illustrations, infographics (21×21), and knowledge comics (educational/biography/tutorial). All from baoyu-skills (Type×Style×Palette or style×tone model)."
version: 2.0.0
...
metadata:
  hermes:
    tags: [article-illustration, infographic, knowledge-comic, creative, image-generation, baoyu, infographics, comics]
    category: creative
...
## §A — Infographics (formerly baoyu-infographic)

Two dimensions: **layout** (information structure) × **style** (visual aesthetics). Freely combine any layout with any style.

| Option | Values |
|---|---|
| Layout | **21** options — bento-grid (default), linear-progression, binary-comparison, comparison-matrix, hierarchical-layers, and 16 more in `references/comic-layouts/` |
| Style | **21** — craft-handmade (default), notion, warm, minimal, blueprint, watercolor, elegant, and 15 more in `references/styles/` |
| Palette | macaron, warm, neon — overrides style's default |
| Aspect | landscape / portrait / square / custom W:H |
| Language | en, ja, zh, … |

Dimension combinations: `type=infographic, style=vector-illustration, palette=macaron`, or use `edu-visual` → type+style+palette in one shot (`references/style-presets.md`).

Content intake: article as text, file path, URL, or topic. The skill produces outline.md, a prompt file per slide, and a PNG per slide — same workflow as the Article Illustrator main section (Steps 1-7 above). Extra analysis refs: `references/infographic-analysis-framework.md`, `references/infographic-base-prompt.md`, `references/infographic-structured-content-template.md`.

Trigger: user says "信息图", "可视化", "高密度信息大图", "infographic", "visual summary".

## §B — Knowledge Comics (formerly baoyu-comic)

Educational comics — flexible **art style × tone** combinations. Trigger: "知识漫画", "教育漫画", "Logicomix-style", "biography comic", "tutorial comic", "comic".

Content intake: source text/file/URL/topic. SKILL flow:
1. Detect reference images (style/palette/scene traits, embedded in per-page prompt as YAML frontmatter)
2. Analyze — content type + purpose + core argument + comic-able beats (see `references/comic-analysis-framework... ` refactor not needed — full reference bank: `references/comic-art-styles/`, `references/comic-presets/`, `references/comic-tones/`, `references/comic-layouts/`, `references/character-template.md`, `references/storyboard-template.md`, `references/auto-selection.md`, `references/ohmsha-guide.md`, `references/partial-workflows.md`.
3. Confirm settings (art style, tone, layout, language, aspect)
4. Generate outline
5. Per page: write prompt file → `image_generate` → download
6. Finalize

Same constraints as parent: labels use article data (never paraphrase), prompt files are mandatory before generation, `image_generate` returns URL (not file), no backend selection.
