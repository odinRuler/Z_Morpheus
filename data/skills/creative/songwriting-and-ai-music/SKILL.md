...
description: "AI music generation umbrella: songcraft (lyrics, structure, parody, Suno prompts), HeartMuLa (open-source Suno-like from lyrics+tags), and AudioCraft (MusicGen/AudioGen text-to-audio)."
version: 2.0.0
author: Hermes Agent + Orchestra Research
license: MIT
tags: [songwriting, music, suno, parody, lyrics, creative, heartmula, audiocraft, musicgen, audiogen, generate, audio]
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [songwriting, music, suno, lyrics, creative, heartmula, audiocraft, musicgen, text-to-audio, audio-generation]
    category: creative
    related_skills: []
    triggers:
      - writing a song, music prompt, suno prompt, parody song
      - AI music generation, generate music from text
---
...
## §A — HeartMuLa: Open-Source Music Generation (formerly heartmula)

HeartMuLa is a family of Apache-2.0 music foundation models comparable to Suno — full songs from lyrics + tags, with multilingual support.

**Models:**
- HeartMuLa (3B/7B) — music LM conditioned on lyrics + tags
- HeartCodec (12.5 Hz codec) — high-fidelity audio reconstruction
- HeartTranscriptor — Whisper-based lyrics transcription
- HeartCLAP — audio-text alignment

**VRAM:** minimum 8 GB with `--lazy_load true` (loads/unloads sequentially); 16 GB+ comfortable. Multi-GPU: `--mula_device cuda:0 --codec_device cuda:1`.

**When to use:** user wants local/offline AI music generation, or wants to generate a song from a set of lyrics they've already written (pairs well with the Suno prompt workflow above — write lyrics here, hand them to HeartMuLa for offline generation).

## §B — AudioCraft: Text-to-Music / Text-to-Sound (formerly audiocraft-audio-generation)

Meta's AudioCraft family — `pip install audiocraft` (or from `git+https://github.com/facebookresearch/audiocraft.git`).
...
wav = m.generate(["upbeat electronic dance music with synth arpeggios"])
torchaudio.save("out.wav", wav[0].cpu(), sample_rate=32000)

# AudioGen — text-tofrom audiocraft.models import AudioGen
a = AudioGen.get_pretrained('facebook/audiogen-medium')
a.set_generation_params(duration=5)
wav = a.generate(["thunderstorm with heavy rain and lightning"])
```

**Model variants** (more → better, needs more VRAM):
- MusicGen: small (300M) / medium (1.5B) / large (3.3B) / melody / stereo / style
- AudioGen: medium (1.5B) text-to-sound

VRAM: small ~2 GB FP16, medium ~4 GB, large ~8 GB. Melody-conditioned generation: load `musicgen-melody`, call `model.generate_with_chroma(descriptions, melody_audio, sr)`. Style transfer: load `musicgen-style`, `model.generate_with_style(descriptions, style_audio, sr)`.

Full reference docs: `references/audiocraft-advanced-usage.md` (training/fine-tuning) and `references/audiocraft-troubleshooting.md` (CUOM, quality).
