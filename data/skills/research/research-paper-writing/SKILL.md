...
description: "Research umbrella: paper-writing pipeline (design→submit for NeurIPS/ICML/ICLR/ACL/AAAI/COLM), arXiv discovery, blog/RSS monitoring via blogwatcher, and Polymarket prediction-market queries."
version: 2.0.0
...
metadata:
  hermes:
    tags: [Research, Paper Writing, Experiments, ML, AI, NeurIPS, ICML, ICLR, ACL, AAAI, COLM, LaTeX, Citations, Statistical Analysis, ArXiv, Blogwatch, RSS, Polymarket, Prediction-Markets]
    category: research
    related_skills: [ml-paper-writing, subagent-driven-development, plan]
...
## §A — arXiv Paper Discovery (formerly arxiv)

Search and retrieve academic papers from arXiv via their free REST API (no key needed). Helper script: `scripts/search_arxiv.py`.

```bash
# Search
curl "https://export.arxiv.org/api/query?search_query=all:transformer+attention&max_results=10&sortBy=submittedDate&sortOrder=descending"
...
```python
import arxiv
results = list(arxiv.Search(query='ti:"Exact Title"', max_results=5,
                            sort_by=arxiv.SortCriterion.SubmittedDate).results())
for r in results: print(r.title, r.entry_id, r.summary)
```

API returns Atom XML — pipe through `grep`/`sed` or parse with `python3 -c "..."`. Use this in Phase 1 (Literature Review) of the pipeline. Combine with `webtopic] site:arxiv.org")` for discovery beyond the API's query syntax.

## §B — Blog & RSS Monitoring (formerly blogwatcher)

Track blog and RSS/Atom feed updates with `blogwatcher-cli` (JulienTant fork of Hyaxia/blogwatcher).
...
# Common ops
blogwatcher feeds add https://example.com/feed.xml
blogwatcher feeds list
blogwatcher articles list --unread
blogwatcher articles read <id>
blogwatcher opml import feeds.opml
```

Auto-discovery works for most blogs even without explicit feed URL — blogwatcher scrapes HTML looking for feed links. Good for research groups' blogs that don't prominently advertise an RSS feed.

## §C — Polymarket Prediction Data (formerly polymarket)

Read-only queries against Polymarket's public REST API — no auth needed. Reference: `references/polymarket-api-endpoints.md`. Helper: `scripts/polymarket.py`.

```bash
# Gamma API — events & markets
curl "https://gamma-api.polymarket.com/markets?limit=10&active=true"
curl "https://gamma-api.polymarket.com/events?tag=politics&limit=5"

# CLOB API — orderbook
curl "https://clob.polymarket.com/price?token_id=<id>&side=buy"
```

Data model: an **Event** contains one or more **Markets**; each market has binary outcome prices that ARE probabilities (price 0.65 ≈ 65% likelihood). The `outcomePrices` field is a JSON-encoded array like `["0.80","0.20"]`. Use for research-into-public-belief tasks (prediction market data as a signal, paper position justification, etc.).
